...
- When adding a new package to an SBOM, its associated `PACKAGECONFIG` entries are now excluded from the diff.
...
- Console output handling has been updated:
  ...
  - **Human-readable** output is now provided by using `--human-readable` argument which replaces
